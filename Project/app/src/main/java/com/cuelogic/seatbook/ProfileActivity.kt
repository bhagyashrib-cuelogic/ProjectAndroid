...
class ProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
     ...
     private fun showProfileInfo(){
         ...
         val query = FirebaseDatabase.getInstance().getReference("Employees").orderByChild("uid").equalTo(currentUserUid)
         ...
         query.addListenerForSingleValueEvent(object:ValueEventListener{
             ...
             override fun onDataChange(snapshot: DataSnapshot) {
                 for(item in snapshot.children){
                     ...
                     query.ref.child(item.key.toString()).setValue(EmployeeData(uid, empName,
                         empEmail,empDesignation,empCueId,"1")).addOnCompleteListener(){
